...
['jpg', 'png', 'webp'].forEach((ext) => {
    describe(`${ext.toUpperCase()} files`, () => {
        test('loads file, metadata filename is correct', async () => {
            const d = await (new Photo(images, `exif.${ext}`)).metadata();
            expect(d.filename).toBe(`exif.${ext}`);
        });

        test('correctly parses image dimensions: landscape', async () => {
        ...
        test('correctly parses image dimensions: square/portrait', async () => {
            const d = await (new Photo(images, `exif.${ext}`)).metadata();
        ...
        test('date uses EXIF creation time', async () => {
...
describe('date parsing', () => {
    test('EXIF with OffsetTime is respected', async () => {
    ...
    test('EXIF with SubSecTime is respected', async () => {
    ...
    test('XMP used when no EXIF', async () => {
    ...
    test('IPTC used when no EXIF and no XMP', async () => {
    ...
    test('IPTC missing TimeCreated', async () => {
    ...
    test('falls back to file creation time when no metadata', async() => {
